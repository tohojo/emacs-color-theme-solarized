...
(defun solarized-color-definitions ()
  (let ((bold        (if solarized-bold 'bold        'unspecified))
    ...
    (let ((bg-back   '(:background back))
          ...
          (fmt-undl-green    `(                      :underline (:color "#859900")))
          (fmt-strike-red    `(                      :strike-through "#dc322f"))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                ...
                (helm-ff-dotted-directory (,@bg-base02 ,@fg-base1))
                (helm-ff-dotted-symlink-directory (,@bg-base02 ,@fg-cyan))
                ...
                ;; notmuch
                (notmuch-crypto-decryption (,@fmt-revr ,@fg-violet))
                (notmuch-crypto-part-header (,@fg-base3))
                (notmuch-crypto-signature-bad (,@fmt-revr ,@fg-red))
                (notmuch-crypto-signature-good (,@fmt-revr ,@fg-green))
                (notmuch-crypto-signature-good-key (,@fmt-revr ,@fg-orange))
                (notmuch-crypto-signature-unknown (,@fmt-revr ,@fg-red))
                (notmuch-search-flagged-face (,@fg-base3))
                (notmuch-tag-added (,@fmt-undl-green))
                (notmuch-tag-deleted (,@fmt-strike-red))
                (notmuch-tag-flagged (,@fg-base3))
                ...
                ;; erc
                (erc-current-nick-face (,@fmt-bold ,@fg-cyan))
                (erc-dangerous-host-face (,@fg-red))
                (erc-direct-msg-face (,@fg-yellow))
                (erc-error-face (,@fg-red))
                (erc-header-line (,@bg-base02 ,@fg-base2))
                (erc-input-face (,@fg-base00))
                (erc-keyword-face (,@fg-base2))
                (erc-my-nick-face (,@fg-cyan))
                (erc-nick-msg-face (,@fmt-bold ,@fg-yellow))
                (erc-notice-face (,@fmt-bold ,@fg-violet))
                (erc-pal-face (,@fmt-bold ,@fg-orange))
                (erc-prompt-face (,@fmt-revr ,@fmt-bold ,@fg-base0))
                (erc-timestamp-face (,@fmt-bold ,@fg-green))
                ;; undo-tree
                ...
                ;; dired+
                (diredp-autofile-name (,@bg-base02))
                (diredp-tagged-autofile-name (,@bg-base02))
                (diredp-compressed-file-name (,@fg-base1))
                (diredp-compressed-file-suffix (,@fg-violet))
                (diredp-date-time (,@fg-base00))
                (diredp-deletion (,@bg-red ,@fg-base3))
                (diredp-deletion-file-name (,@fg-red))
                (diredp-dir-heading (,@fg-base1 ,@bg-base02))
                (diredp-dir-name (,@fg-blue))
                (diredp-file-name (,@fg-base1))
                (diredp-file-suffix (,@fg-base0))
                (diredp-flag-mark (,@bg-green ,@fg-base02))
                (diredp-flag-mark-line (:inherit secondary-selection))
                (diredp-ignored-file-name (,@fg-base01))
                (diredp-mode-line-flagged (,@fg-red))
                (diredp-mode-line-marked (,@fg-magenta))
                (diredp-number (,@fg-base1))
                (diredp-symlink (,@fmt-bold ,@fg-cyan))
                (diredp-dir-priv (,@fg-cyan))
                (diredp-exec-priv (,@fg-yellow))
                (diredp-link-priv (,@fg-blue))
                (diredp-no-priv (,@fg-base0))
                (diredp-other-priv (,@fg-cyan))
